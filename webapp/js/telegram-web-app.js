...
(function () {
  ...
  function urlAppendHashParams(url, addHash) {
    ...
    if (curHash.indexOf('=') >= 0 || curHash.indexOf('?') >= 0) {
      ...
      // https://game.com/#path?query -> https://game.com/#path?tgShareScoreUrl=etc
      return url + '&' + addHash;
    }
...
(function () {
  ...
  WebApp.showPopup = function (params, callback) {
    ...
    if (typeof params.buttons !== 'undefined') {
      ...
      for (var i = 0; i < params.buttons.length; i++) {
        ...
        btn_id = strTrim(btn_id);
        if (btn_id.length > 64) {
          console.error('[Telegram.WebApp] Popup button id is too long', btn_id);
          throw Error('WebAppPopupParamInvalid');
        }

        var btn = {id: btn_id};
        var btn_type = button.type;
        if (typeof btn_type === 'undefined' ||
            btn_type === null) {
          btn_type = 'default';
        }
        btn_type = strTrim(btn_type);
        switch (btn_type) {
          case 'default':
          case 'destructive':
          case 'ok':
          ...
          case 'cancel':
            ...
            break;
          default:
            console.error('[Telegram.WebApp] Popup button type is invalid', btn_type);
            throw Error('WebAppPopupParamInvalid');
        }
        var btn_text = '';
        if (typeof button.text !== 'undefined') {
          btn_text = strTrim(button.text);
        }
        if (btn_text.length == 0 &&
            (btn_type == 'default' || btn_type == 'destructive')) {
          console.error('[Telegram.WebApp] Popup button text is required for type ' + btn_type, button.text);
          throw Error('WebAppPopupParamInvalid');
        }
        if (btn_text.length > 64) {
          console.error('[Telegram.WebApp] Popup button text is too long', btn_text);
          throw Error('WebAppPopupParamInvalid');
        }
        if (btn_text.length > 0) {
          btn.text = btn_text;
    ...
    if (buttons.length > 3) {
      console.error('[Telegram.WebApp] Popup buttons count is invalid', buttons.length);
      throw Error('WebAppPopupParamInvalid');
    }
    if (buttons.length == 0) {
    ...
    WebView.postEvent('web_app_show_popup', false, popup_params);
  };
  ...
  WebApp.showScanQrPopup = function (params, callback) {
    ...
    WebView.postEvent('web_app_show_scan_qr_popup', false, popup_params);
  };
  ...
  WebApp.readTextFromClipboard = function(callback) {
    ...
    WebView.postEvent('web_app_read_text_from_clipboard', false, {req_id: req_id});
  };
  WebApp.requestWriteAccess = function(callback) {
    ...
    WebView.postEvent('web_app_request_write_access', false, {from_bot: true});
  };
  ...
  WebApp.downloadFile = function(params, callback) {
    ...
    if (webAppDownloadFileRequested) {
      console.error('[Telegram.WebApp] File is already downloading');
      throw Error('WebAppFileDownloading');
    }
    var url = params.url;
    var filename = params.filename || url.substr(url.lastIndexOf('/') + 1);
    webAppDownloadFileRequested = {
      callback: callback
    };
    WebView.postEvent('web_app_download_file', false, {url: url, filename: filename});
  };
  ...
  WebApp.close = function (options) {
    ...
    if (typeof options !== 'undefined' &&
        options.return_after_close) {
      params.return_after_close = true;
    }
    WebView.postEvent('web_app_close', false, params);
  };
  WebApp.closeImmediately = function () {
    if (!versionAtLeast('7.6')) {
      console.error('[Telegram.WebApp] Method closeImmediately is not supported in version ' + webAppVersion);
      throw Error('WebAppMethodUnsupported');
    }
    WebView.postEvent('web_app_close', true);
  };

  window.Telegram.WebApp = WebApp;

  // App backward compatibility
  window.Telegram.WebApp.sendDataUnsafe = function(data) {
    if (!versionAtLeast('7.0')) {
      console.error('[Telegram.WebApp] Method sendDataUnsafe is not supported in version ' + webAppVersion);
      throw Error('WebAppMethodUnsupported');
    }
    WebView.postEvent('web_app_data_send', false, {data: data});
  };
  ...
  if (initParams.tgWebAppDebug) {
    window.addEventListener('error', function(e) {
      if (e.error) {
        var err_text = e.message + "\n" + e.error.stack;
      } else {
        var err_text = e.message;
      }
      WebView.postEvent('web_app_debug_log', false, {message: '[Error] ' + err_text});
    });
    var console_log = console.log;
    var console_warn = console.warn;
    var console_error = console.error;
    var console_debug = console.debug;
    console.log = function() { WebView.postEvent('web_app_debug_log', false, {message: '[Log] ' + JSON.stringify(Array.from(arguments))}); console_log.apply(this, arguments); };
    console.warn = function() { WebView.postEvent('web_app_debug_log', false, {message: '[Warn] ' + JSON.stringify(Array.from(arguments))}); console_warn.apply(this, arguments); };
    console.error = function() { WebView.postEvent('web_app_debug_log', false, {message: '[Error] ' + JSON.stringify(Array.from(arguments))}); console_error.apply(this, arguments); };
    console.debug = function() { WebView.postEvent('web_app_debug_log', false, {message: '[Debug] ' + JSON.stringify(Array.from(arguments))}); console_debug.apply(this, arguments); };
  }
